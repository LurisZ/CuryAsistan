...
client.on('message', msg => {
  if (msg.content.toLowerCase() === 'sa') {
    msg.reply('<a:tac:743420016053190658> Aleyküm selam Babba <a:tac:743420016053190658>');
  }
});
...
client.on('ready', () =>{
  client.channels.get('743401440894255174').join()
  })
client.on("guildMemberAdd", async member => {


const turkcode = new Discord.RichEmbed()

.setColor("RANDOM")
.addField("**Sunucumuza Hoşgeldin Kralım <a:tac:743420016053190658> **", "<a:zil:743486584443109418> Kuralları Okumayı Unutma <a:zil:743486584443109418> ")
.setFooter("Bu Sunucu CURY BOT Tarafından Korunmaktadır") 

member.send(turkcode); 

})
client.on('guildMemberAdd', async(member) => {
   const kanal = `Son Üye: ${member.user.username}`
   let channel = client.channels.get("744082106086457425")//KANAL ID
